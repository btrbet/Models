...
from utils import (
    ...
    get_pitching_splits_for_player_for_year,
)
...
def predict_strikeout_probability(data, line, use_weights=True):
    ...
    # Define weights
    weights = {"7d": 1.00, "14d": 0.75, "28d": 0.50, "season": 0.25}
...
def main():
    ...
    splits, info_dict = get_pitching_splits_for_player_for_year(
        player["key_bbref"][0], YEAR
    )
    ...
    print(
        f"{args.first} {args.last} had {last_28_days_so_per_game} SO/G in the last 28 days"
    )
    ...
    print(
        f"{args.first} {args.last} had {last_14_days_so_per_game} SO/G in the last 14 days"
    )
    ...
    print(
        f"{args.first} {args.last} had {last_7_days_so_per_game} SO/G in the last 7 days"
    )
    ...
    if args.line is not None:
        ...
        l28_prob_gt_line = round(
            1 - poisson.cdf(rounded_line, last_28_days_so_per_game), 4
        )
        season_prob_gt_line = round(
            1 - poisson.cdf(rounded_line, season_so_per_game), 4
        )
        ...
        print(
            f"Probability of o{args.line} SO in the next game based on last 28 days: {(l28_prob_gt_line * 100.00):.2f}% ≈ {l28_gt_odds}"
        )
        print(
            f"Probability of o{args.line} SO in the next game based on season: {(season_prob_gt_line * 100.00):.2f}% ≈ {season_gt_odds}"
        )
        print("")

        # Calculate the probability of getting < {line} SO in the next game
        l28_prob_leq_line = round(
            poisson.cdf(rounded_line, last_28_days_so_per_game), 4
        )
        ...
        print(
            f"Probability of u{args.line} SO in the next game based on last 28 days: {(l28_prob_leq_line * 100.00):.2f}% ≈ {l28_leq_odds}"
        )
        print(
            f"Probability of u{args.line} SO in the next game based on season: {(season_prob_leq_line * 100.00):.2f}% ≈ {season_leq_odds}"
        )
        print("")
        ...
        data = {
            ...
            "14d": {
                "G": last_14_days_g,
                "K": last_14_days_so,
            },  # last 14d (not including 7d)
            "28d": {
                "G": last_28_days_g,
                "K": last_28_days_so,
            },  # last 28d (not including 14d)
            "season": {
                "G": current_season_g,
                "K": current_season_so,
            },  # rest of season beyond 28d
        }
        prob_under_line, prob_over_line = predict_strikeout_probability(
            data, rounded_line
        )
        prob_under_odds = convert_prob_to_american_odds(prob_under_line)
        prob_over_odds = convert_prob_to_american_odds(prob_over_line)
        print(
            f"Probability of o{args.line} SO in the next game: {(prob_over_line * 100.00):.2f}% ≈ {prob_over_odds}"
        )
        print(
            f"Probability of u{args.line} SO in the next game: {(prob_under_line * 100.00):.2f}% ≈ {prob_under_odds}"
        )
        print("")
